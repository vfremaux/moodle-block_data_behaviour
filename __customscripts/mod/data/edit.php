...
// CHANGE+.
require_once($CFG->dirroot.'/blocks/data_behaviour/xlib.php');

if (is_dir($CFG->dirroot.'/mod/data/field/calendar')) {
    $PAGE->requires->js('/mod/data/field/calendar/js/dhtmlxCalendar/codebase/dhtmlxcalendar.js', true);
    $PAGE->requires->js('/mod/data/field/calendar/js/dhtmlxCalendar/codebase/dhtmlxcalendar_locales.js', true);
    $PAGE->requires->css('/mod/data/field/calendar/js/dhtmlxCalendar/codebase/dhtmlxcalendar.css');
    $PAGE->requires->css('/mod/data/field/calendar/js/dhtmlxCalendar/codebase/skins/dhtmlxcalendar_dhx_omega.css');
    $PAGE->requires->css('/mod/data/field/calendar/js/dhtmlxCalendar/codebase/skins/dhtmlxcalendar_dhx_web.css');
    $PAGE->requires->css('/mod/data/field/calendar/js/dhtmlxCalendar/codebase/skins/dhtmlxcalendar_dhx_terrace.css');
}
// CHANGE-.
...
$fpage = optional_param('page', '', PARAM_INT);    // Page number from format page.
// CHANGE-.
...
if (isguestuser()) {
    // CHANGE+.
    if ($return == 'coursepage') {
        redirect(new moodle_url('/course/view.php', array('id' => $course->id, 'page' => $fpage)));
    } else {
        redirect(new moodle_url('/mod/data/view.php', array('d' => $data->id)));
    }
    // CHANGE-.
}
...
// CHANGE+ : Simplify process from page format.
if ($return != 'coursepage') {
    if (empty($manager)) {
        include($CFG->dirroot.'/mod/data/tabs.php');
    } else {
...
// CHANGE+ : add return router.
echo '<input name="return" value="'.$return.'" type="hidden" />';
echo '<input name="page" value="'.$fpage.'" type="hidden" />';
// CHANGE- : add return router.
